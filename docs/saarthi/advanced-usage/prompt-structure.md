...
Advanced users can create custom system prompts for specific modes by placing a `.saarthi/system-prompt-<mode_slug>` file in their workspace. When present, Roo uses this file instead of generating the standard system prompt sections, allowing for complete customization of Roo's behavior in that mode.
