...
- **[Datasources](/docs/microservice-meta-framework/datasources/overview.md):** Datasources are locations where data can be stored or read from. Developers can use [pre-built](/docs/microservice-meta-framework/datasources/datasource-plugins.md) datasource types or create their [custom plugin](/docs/microservice-meta-framework/datasources/create-custom-datasource.md). For example API datasource (another microservice or third party), datastores (RDBMS, document, key-value), file system, S3 storage, message bus, cache etc. A microservice can use multiple datasources. 
...
The framework promites setting up of environment variables in a pre-defined YAML file. Though the developer can also allow access by other means via a .env file or setting them up manually. Further [configurations](/docs/microservice-meta-framework/config/overview.md) are to be written in `/config` folder. These variables are accessible in 
