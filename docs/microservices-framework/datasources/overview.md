...
`datasource.mongo.User.findOne` is the data source function, which can be described as below:
  ...
  - `mongo`: name of data source,this can be any data base that you select can checkout [database list](/docs/microservices-framework/getting-started/create-blog-project.md#prisma-supports-wide-range-of-databases)
  - `User`: entity name
