 ...
 **We set `NODE_ENV` to prod so that we can see OTEL Format of logging.**
 ...
 **Similary we set `NODE_ENV` to dev so that we can see pinopretty Format of logging, for more readabilty of developer**
...
By setting NODE_ENV to prod, you'll see logs in OTELO format suitable for production environments. Conversely, setting it to dev will display logs in PrettyPrint format for better readability during development. Adjust the format based on your environment needs.
