...
- **Datasources:** Datasources are locations where data can be stored or read from. Developers can use [pre-built](data_sources/datasource_plugins.md) datasource types or create their [custom plugin](data_sources/custom_data_source.md). For example API datasource (another microservice or third party), datastores (RDBMS, document, key-value), file system, S3 storage, message bus etc. A microservice can use multiple datasources. 
